...
# Esto montará el directorio MasterARM o MasterAMD respectivamente, y se abrirá una terminal dentro del contenedor con ese directorio como raíz. 
# Desde ahí, se pueden ejecutar los comandos necesarios para compilar y correr el código dentro del contenedor.
dockerarm:
	docker run -v "$(shell pwd)/MasterARM:/root" -w /root --privileged -ti $(DOCKER_IMAGE)

dockeramd:
	docker run -v "$(shell pwd)/MasterAMD:/root" -w /root --privileged -ti $(DOCKER_IMAGE)